...
MAX_READ =[500, 10000] #topic, message
READ_BUF = [bytearray(MAX_READ[0]), bytearray(MAX_READ[1])]
...
class MQTTProto:
    ...
    def __init__(self, subs_cb, puback_cb, suback_cb, pingresp_cb, sock_cb=None):
        ...
        self.read_buf_view = [memoryview(READ_BUF[0]) , memoryview(READ_BUF[1])]
    ...
    async def connect(
        ...
        try:
            ...
            try:
                self._sock, self._wsock = await asyncio.wait_for(open_connection(addr, ssl), 30)
        ...
        try:
            ...
            try:
                try:
                    resp = await asyncio.wait_for(self._as_read(4), 30)
                except asyncio.TimeoutError:
                    raise OSError(-1, CONN_TIMEOUT)
            except OSError as e:
    ...
    async def _as_read(self, n, buf_id=0):
        """
        ...
        """
        if n <= 0:
            raise ValueError("incorrect read len")
        if n < 10:
            return await self._as_read_orig(n)

        if n > MAX_READ[buf_id]:
            raise ValueError(f"Read buffer too small for {n}")

        read = 0
        while self._sock and read < n:
            buf =  self.read_buf_view[buf_id][read:n]
            try:
                got = await asyncio.wait_for(self._sock.readinto(buf), self.timeout)
            except asyncio.TimeoutError:
                raise OSError(-1, CONN_TIMEOUT)
            if got == 0:
        ...
        return self.read_buf_view[buf_id][:n]
    ...
    async def _as_read_orig(self, n):
        # Note: uasyncio.Stream.read returns short reads
        read = 0
        read_buf = b'';
        while self._sock and read < n:
            # read missing amt
            got = await self._sock.read(n - read)
            if got is None:
                continue
            if len(got) == 0:
                raise OSError(-1, CONN_CLOSED)
    ...
    async def _as_write(self, bytes_wr, drain=True):
        print('_as_write', len(bytes_wr))
        if self._sock is None:
            raise OSError(-1, CONN_CLOSED)
        try:
            if len(bytes_wr) >= 200 and drain:
                await asyncio.wait_for(self._sock.drain_buf(bytes_wr), self.timeout)
            else:
                if bytes_wr != b"":
                    self._sock.write(bytes_wr)
                if drain:
                    print('drain')
                    await asyncio.wait_for(self._sock.drain(), self.timeout)
        except asyncio.TimeoutError:
            raise OSError(-1, CONN_TIMEOUT)
    ...
    async def publish(self, msg, dup=0):
        ...
        single = single and hdrlen + mlen <= 200  # slightly conservative MSS
